...
#[udf]
pub fn format_5(template:&str, value1: &str,value2: &str,value3:&str,value4:&str,value5:&str) ->String{
...
pub fn format(template: &str, values: &[&str]) -> String {
    ...
    match strfmt(template, &vars) {
        Ok(result) => result,
        Err(e) => {
            let error_msg_vals = values.iter()
                                       .map(|v| v.to_string())
                                       .collect::<Vec<_>>()
                                       .join(", ");
            format!("Formatting with template {} and values {} caused an error: {:?}", template, error_msg_vals, e)
        }
    }
}
